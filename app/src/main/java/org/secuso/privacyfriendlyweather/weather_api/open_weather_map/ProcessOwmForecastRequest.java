...
public class ProcessOwmForecastRequest implements IProcessHttpRequest {
     ...
     */
    ProcessOwmForecastRequest(Context context, int cityId) {
    ...
    private void possiblyUpdateWidgets(int cityID) {
        ...
        for (int widgetID : ids1day) {
            ...
            if (cityID == prefs1.getInt(WeatherWidget.PREF_PREFIX_KEY + widgetID, -1)) {
                ...
                updateWidget(widgetID, cityID, views, 1);
            }
        }
        ...
        for (int widgetID : ids3day) {
            //check if city ID is same
            if (cityID == prefs3.getInt(WeatherWidgetThreeDayForecast.PREFS_NAME + widgetID, -1)) {
                //perform update for the widget
                RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.weather_3day_widget);
                updateWidget(widgetID, cityID, views, 3);
            }
        }
        ...
        for (int widgetID : ids5day) {
            //check if city ID is same
            if (cityID == prefs5.getInt(WeatherWidgetFiveDayForecast.PREFS_NAME + widgetID, -1)) {
                //perform update for the widget
                RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.weather_5day_widget);
                updateWidget(widgetID, cityID, views, 5);
            }
        }
    ...
    private void updateWidget(int widgetId, int cityId, RemoteViews views, int widgetType) {
